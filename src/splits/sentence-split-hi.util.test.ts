import { splitIntoSentencesSimpleHi } from "./sentence-split-hi.util";
import { expect, describe, it } from "bun:test";

describe("splitIntoSentencesSimpleHi", () => {
  it("expect to 5 sentences", () => {
    ...
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(3);
  });
  it("do not split डॉ. एलेना", () => {
    const input = `उनका नाम डॉ. एलेना रोड्रिगेज था`;
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(1);
  });
  it("do not split name R. Kelly", () => {
    let input = `R. Kelly`;
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(1);
    input = `यह तथ्य कि इसे आर. केली ने लिखा और इतने न्यूनतम लेकिन प्रभावशाली व्यवस्था के साथ निर्मित किया गया`;
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(1);
  });
  it("do not split Dr. Linh", () => {
    let input = `Dr. Linh`;
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(1);
    input = `वह मेरी मौसी, डॉ. लिन्ह हैं`;
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(1);
  });
  it("do not split R.R Martin", () => {
    let input = `R.R Martin`;
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(1);
    input = `आर.आर. मार्टिन की `;
    expect(splitIntoSentencesSimpleHi(input).length).toEqual(1);
  });
